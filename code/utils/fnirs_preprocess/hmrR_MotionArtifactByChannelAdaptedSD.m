% [tInc,tIncCh] = hmrR_MotionArtifactByChannelAdaptedSD(data, probe, mlActMan, mlActAuto, tIncMan, tMotion, tMask, STDEVthresh, AMPthresh)
%
% UI NAME:   
...
%     that stdev_thresh * std_diff over the time interval tMotion, then
%     this time point is marked as a motion artifact. The standard deviation is
%     determined for each channel independently. To obtain std_diff, first 
%     the calculation of standard deviation is performed over 4-second segments, then
%     the obtained values are sorted, finally, only the standard deviations of the first 3
%     30% of segments is averaged (the noise-free signal).
...
%
% Modified by CI & SB 3/29/2024 std_diff is computed only on the noise-free physiology of the signal.
%
% TO DO:
% Consider tIncMan

function [tInc, tIncCh] = hmrR_MotionArtifactByChannelAdaptedSD(data, probe, mlActMan, mlActAuto, tIncMan, tMotion, tMask, std_thresh, amp_thresh)
...
for iBlk=1:length(data)
    ...
    % Calculate the diff of d to to set the threshold if necessesary
    diff_d = diff(d);
    ...
    %% -- modified by ML
    idxStart = find(diff(tIncMan{iBlk})==1)+1;
    idxEnd = find(diff(tIncMan{iBlk})==-1);
    extend = 4*fs;
...
%         std_diff = std(d(2:end,lstActTmp)-d(1:end-1,lstActTmp),0,1);
        ...
        %% -- modified by CI and SB -- modified by ML --
        % Calculation of standard deviation over 4-second segments.
        % std_diff_W=movstd(d(:,lstActTmp),4*fs);
        std_diff_W = [];
        for iTrial = 1:nTrials
            std_diff_W = [std_diff_W; movstd(d(:,lstActTmp),4*fs)];
        end
        % All obtained values are sorted         
        std_diff_sort=sort(std_diff_W);
        % only the standard deviations of the first 30% of segments is
        % averaged
        Perc30=round(size(std_diff_sort,1)/3);
        std_diff=mean(std_diff_sort(1:Perc30,:),1);
        %% --       
