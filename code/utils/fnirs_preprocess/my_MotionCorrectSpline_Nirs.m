...
for ii = 1:length(lstAct)
    ...
    if ~isempty(lstMA)
        ...
        if lstMs(1)>1
            meanPrev = dodSpline(lst(1)-1, idx_ch); % last point
            meanCurr = dodSpline(lst(1), idx_ch); % first point
            dodSpline(lst,idx_ch) = dodSpline(lst,idx_ch) - meanCurr + meanPrev;

        else
            meanCurr = dodSpline(lst(end),  idx_ch);
            meanNext = dodSpline(lst(end)+1, idx_ch);
        ...
        for kk=1:(nbMA-1)
            ...
            meanPrev = dodSpline(lst(1)-1, idx_ch);
            meanCurr = dod(lst(1), idx_ch);
            ...
            meanPrev = dodSpline(lst(1)-1, idx_ch);
            meanCurr = dodSpline(lst(1), idx_ch);
