function trl = my_trialfun_exp1(cfg)
...
rating_file_idx = contains({files.name},'exp1');
if sum(rating_file_idx) == 1
    rating_file = files(rating_file_idx).name;
elseif sum(rating_file_idx) > 1
    error('Detected more than one rating file for [Experiment 1].')
elseif sum(rating_file_idx) == 0
    error('Detected no rating file for [Experiment 1].')
end
ratings = readtable([folder,'/',rating_file]);
ratings = ratings(ratings.percCompleted >0, :);
...
for i=1:length(event)
    if strcmp(event(i).type, [cfg.trialdef.prefix,'51']) % Perception trial start (trigger 51)
        begsample       = event(i).sample - pretrial; % start of segment
        offset          = -pretrial; % how many samples before trial
        cur_task        = 'Perception';
        in_trial        = true;
    elseif in_trial
        if strcmp(event(i).type, [cfg.trialdef.prefix,'52']) % trial end
            endsample       = event(i).sample + posttrial; % end of segment
            trial(end+1,:)  = [begsample, endsample, offset];
            task{end+1,1}   = cur_task;
        elseif strcmp(event(i).type, [cfg.trialdef.prefix, '50']) % trial skip
            endsample       = event(i).sample + posttrial;
            trial(end+1,:)  = [begsample, NaN, offset];
            task{end+1,1}   = cur_task;
        end
        in_trial        = false;
    end
end
% handle [trial skip] situation -> delete it
idx_delete = find(isnan(trial(:,2)));
if ~isempty(idx_delete)
    ratings(idx_delete+session_start_idx-1,:)   = []; % both in rating recordings
    trial(idx_delete,:)                         = []; % and in trial definings
    task(idx_delete,:)                          = [];
end
...
trl.Properties.VariableNames{'videoEmotion'} = 'targetEmotion';
ori_cols = width(trl);
