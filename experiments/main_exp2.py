...
from utils_exp import Experiment2
import serial
import sys

# Initialize the program (press [Ctr]+[Alt]+[Q] to quit)
exp = Experiment2(exp_dir=sys.path[0],
...
exp.show_instruction(wait='space', text=exp.recall_guide('neutral'))
exp.recall_task(duration=90, prepare=3, log=False)
exp.get_ratings(log=False)
exp.show_instruction(wait='space', text=exp.tell_guide)
exp.tell_task(duration=180, prepare=3, log=False, save=False)
...
for block in exp.blocks:
    exp.block_idx += 1
    for trial in block:
        print(f'------ Target Emotion: {trial} ------')
        exp.trial_idx += 1
        exp.trial_log.at[exp.trial_idx, 'targetEmotion'] = trial
        exp.show_instruction(wait='space', text=exp.recall_guide(trial))
        print('Recalling start.')
        exp.recall_task(duration=90, prepare=3)
        print('Recalling end.')
        successfully_recall = exp.recall_check()
        if successfully_recall:
            print('Rating start.')
            exp.get_ratings()
            print('Rating end.')
            exp.show_instruction(wait='space', text=exp.tell_guide)
            print('Telling start.')
            exp.tell_task(duration=180, prepare=3, emotion=trial)
            print('Telling end.')
        else:
            print('No corresponding event.')
            exp.show_instruction(wait=3, text='未回忆起相关事件，为您跳过后续环节')
    if exp.block_idx < 3:
        exp.show_instruction(wait='space', text=exp.math_guide)
        exp.show_fixation(wait=3)
        exp.do_math()
